...
Variable::Variable()
{
	std::cerr << "Default Variable constructor was called...(It may be error)\n";
  assert(1==0);
  return;  // do nothing now
}

Variable::Variable(const char *name, int type)
{
  varName = name;
  varType = type;
...
int Variable::GetEtype(void)
{
  ...
  if(feelfem_debug) {
	  std::cerr << "CHECK GetEtype " << varName << std::endl;
  }
...
Quadrature *Variable::GetQuadraturePtr(void)
{
  ...
  case VAR_EWISE_A:
    if(ewiseType == EWISE_TYPE_GAUSSPOINT) {
      return(qPtr);
	    ...
	    std::cerr << "Try to get quadPtr from ewise-interpolation variable.\n";
      assert(1==0);
    }
    break;
  ...
  case VAR_CONST:
  case VAR_FEM:
    std::cerr << "Variable::GetQuadraturePtr not quadrature variable\n";
    assert(1==0);
...
Element *Variable::GetElementPtr(void)
{
  ...
  case VAR_EWISE_A:
    if(ewiseType == EWISE_TYPE_INTERPOLATION) {
      return(ePtr);
	    ...
	    std::cerr << "Try to get elemPtr from ewise-gausspoint variable.\n";
      assert(1==0);
    }
    break;
  ...
  case VAR_INT:
  case VAR_CONST:
    std::cerr << "Variable::GetQuadraturePtr not element variable\n";
    assert(1==0);
...
{
	std::cout << "(dbg:Variable=[" << var.GetName() << "]";
  return cout;
}
