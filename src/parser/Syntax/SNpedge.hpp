...
public:
  SNpedge( const char *nm, SNunit *ptr, Expression *xp,Expression *yp, 
	   const char *pvar, Expression *from,Expression *to,int n) 
  ...
  // with start omit
  SNpedge( int omit,const char *nm, SNunit *ptr, Expression *xp,Expression *yp, 
	   const char *pvar, Expression *from,Expression *to,int n) 
    : name(nm),SNunit("pedge",SN_PEDGE) {
    identifierLst = (SNidentifierlist *)ptr;
    xExpr         = xp ;
  ...
  // Without two points
  SNpedge( const char *nm,   Expression *xp,Expression *yp, 
	   const char *pvar, Expression *from,Expression *to,int n) 
    : name(nm),SNunit("pedge",SN_PEDGE) {
    identifierLst = 0;
    xExpr         = xp ;
  ...
  SNpedge( const char *nm, SNunit *ptr, Expression *xp,Expression *yp, 
	   const char *pvar, Expression *from,Expression *to) 
    : name(nm),SNunit("pedge",SN_PEDGE) {
    identifierLst = (SNidentifierlist *)ptr;
    xExpr         = xp ;
    yExpr         = yp ;
