 ...
 *
 *  Filename : MakeBoundaryElement_tetra.cpp
 *  Date     : 2002/04/01
 *  Modified : 
 *  
 *  Purpose  : Boundary element for tetraangle element
 *
 *
...
void Element::MakeBoundaryElement_tetra(TermConvert &tc)
{
  int boundaryElementType    = ELEMENT_TYPE_TRI;     // tetra dependent
  ...
  for(itrP.init(); !itrP ; ++itrP ) {
    ...
    // restrict xy-plane  ( TETRA type dependent)
    subsExprGinacEvaluate( 2, 0.0);  // t == 0
    ...
    newIPtr->SetExprStr( (const char *)exprG);
