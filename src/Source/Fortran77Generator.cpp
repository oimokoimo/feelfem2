...
void Fortran77Generator::flushSource(void)
{
  ...
  while(stringLength(srcPtr) > 0) {
    ...
    f77buf[columns+7] = 0;
    std::cerr << "stringLength(f77buf) = " <<stringLength(f77buf) << "\n";
    fprintf(fp,"%s\n",f77buf);
  }
...
void Fortran77Generator::COMMENTlong(const char *com)
{
  char buf[BUFSIZ];
  writeSource("*-----");
  sprintf(buf,"* %s",com);
 ...
 *---------------------------------------*/
void Fortran77Generator::mallocF77(const char *varname,
				   const char *comment,
				   const char *size,
				   const char *type     )
{
...
void Fortran77Generator::mallocF77imode(const char *varname,
				        const char *comment,
				        const char *size,
				        const char *type)
{
