...
{
  std::cerr << "Default MakefileObject constructor was called...(It may be error)\n";
...
MakefileObject::~MakefileObject() = default;
...
int MakefileObject::getLanguageType(const char *name) //this private function(char*)
{
  ...
  return 1;  //abortExit exit program
...
{
  fprintf(fp,"%s",(const char *)objectName);

  return;
}


const char *MakefileObject::getObjectName(void)
{
  return(objectName);
}

  
std::ostream & operator << (std::ostream &cout, MakefileObject &mobj )
{
