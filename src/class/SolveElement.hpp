...
public:
  ...
  // const-correct inline getter (header-only change)
  int GetIelemNodes() const { return ielemNodes; }
  ...
  // keep return-by-value as-is (do not change ABI / call sites)
  const orderedPtrList <Variable *>GetVariablePtrLst(void) const {
    return ( varPtrLst );
  }

  const list <Variable *>GetUnknownVariablePtrLst(void) const {
    return ( unknownVarPtrLst);
  }

  const list <string>GetEcalCallValueLst(void) const {
  ...
  // existing non-const reference accessor (kept)
  orderedList<string> &GetEcalIntegrandTermLst() {
    return(ecalIntegrandTermLst);
  }
  // const overload (header-only, non-breaking)
  const orderedList<string> &GetEcalIntegrandTermLst() const {
    return(ecalIntegrandTermLst);
  }
