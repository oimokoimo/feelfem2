 ...
 *
 *  Filename : feelfem90DRAMA_ewquad.cpp
 *  Date     : 2003/02/10 (copied from feelfem90_ewquad.cpp)
 *  Modified : 
 *  
 *  Purpose  : coAssign routine header/footer
 *
 *
...
void PM_feelfem90DRAMA::DoAssignParametersEwQuad(Assign *asPtr)
{
  COMMENTlong("Parameters for EwQuad assignment");
  ...
  // for NDP, number of nodes
  pushInteger4pm();
  pushSource("NDP =");
  pushSourceInt(asPtr->GetNodesEwiseQuad());
  flushSource();
  com();
...
void PM_feelfem90DRAMA::DoAssignVariableDefinitionsEwQuad(Assign *asPtr)
{
  writeSource("! auto Variable Definitions");


  // variable for ex(NDP), etc. is defined in feelfem90DRAMA_assign.cpp
  // in general GEOM_ELEM type 
...
void PM_feelfem90DRAMA::DoAssignInitializeEwQuad(Assign *asPtr)
{
  comment();
...
void PM_feelfem90DRAMA::DoAssignLoopStartEwQuad(Assign *asPtr)
{
  writeSource("do i=1,nelem");
  return;
}

void PM_feelfem90DRAMA::DoAssignAssignmentEwQuad(Assign *asPtr)
{
  // for x,y,z
  ...
  pushSource("(1:");
  pushSourceInt(leftVarPtr->GetEWISEFreedom());
  pushSource(",i)");
...
void PM_feelfem90DRAMA::DoAssignLoopEndEwQuad(Assign *asPtr)
{
  writeSource("end do");
  com();
  return;
...
// CoRoutines
void PM_feelfem90DRAMA::DoAssignCoRoutineHeaderEwQuad( Assign *asPtr)
{
  // module sentence
...
void PM_feelfem90DRAMA::DoAssignCoRoutineParametersEwQuad  ( Assign *asPtr)
{
  comment();
    // for NPG, number of gauss points
  Quadrature *qPtr = asPtr->GetQuadraturePtr();
...
void PM_feelfem90DRAMA::DoAssignCoRoutineVariableDefinitionsEwQuad ( Assign *asPtr)
{
...
void PM_feelfem90DRAMA::DoAssignCoRoutineQuadratureSettingsEwQuad  ( Assign *asPtr)
{
  ...
  for(int ip=0;ip<points;ip++) {
    ...
    // this is for integral
    if(asPtr->GetLeftVarAssignType() == AS_VAR_TYPE_DOUBLE_INTEGRAL) {
      pushSource("  wt");
      pushSource("(");
      pushSourceInt( ip+1 );
      pushSource(") =");
    ...
    if(asPtr->GetLeftVarAssignType() == AS_VAR_TYPE_DOUBLE_BINTEGRAL) {
      assert(1==0);  // may be same as above
    }
...
void PM_feelfem90DRAMA::doAssignEwquadCalcInterpolation(Element *ePtr,
 				                   int      type )
{
...
void PM_feelfem90DRAMA::DoAssignCoRoutineStatementEwQuad( Assign *asPtr)
{
  ...
  // assign statement
  doAssignEwiseQuad(asPtr);

  writeSource("end do");
  return;
}

void PM_feelfem90DRAMA::doAssignEwiseQuad(Assign *asPtr)
{
...
void PM_feelfem90DRAMA::doAssignEwquadEvalAtGaussP(Assign *asPtr)
{
  EwiseQuadEcalInfo *ewqeiPtr = asPtr->GetEwQEcalInfoPtr();  
...
void PM_feelfem90DRAMA::doAssignEwquadRQconv(Assign *asPtr)
{
  EwiseQuadEcalInfo *ewqeiPtr = asPtr->GetEwQEcalInfoPtr();
  listIterator <Element *>itrE0( ewqeiPtr -> GetUsedElementPtrList());
  listIterator <Element *>itrE1( ewqeiPtr -> GetUsedD1ElementPtrList());
...
void PM_feelfem90DRAMA::doAssignEwquadJacobian(Assign *asPtr)
{
  EwiseQuadEcalInfo *ewqeiPtr = asPtr->GetEwQEcalInfoPtr();
...
void PM_feelfem90DRAMA::pushEwquadJacobiSub(int freedom,int *ndPattern,
				       const char *coor, 
...
void PM_feelfem90DRAMA::pushEwquadDiffSub_1st(Element *ePtr,
