 ...
 *
 *  Filename : aistpcp90_dirichlet.cpp
...
void PM_aistpcp90::pushDirichletRoutineName(int solveNo, int dcondNo)
{
  char buf[BUFSIZ];
...
void PM_aistpcp90::F90useDirichletPM()
{
  F90useFEMDataType();
  F90useAlways();
...
const char *PM_aistpcp90::GetDirichletRoutineName(int solveNo, int dcondNo)
{
...
void PM_aistpcp90::DoDirichletArgumentVariableDefinitionsPM(Dirichlet *dPtr)
{
  writeInteger4In("NDF_PCP");
...
void PM_aistpcp90::doDirichletAutoVariableDefinitionsPM(Dirichlet *dPtr)
{
...
void PM_aistpcp90::DoDirichletReturnSequencePM(Dirichlet *dPtr)
{
  com();
...
void PM_aistpcp90::doSetDirichletValue(Dirichlet *dPtr)
{
  pushSource( "  u = ");
...
void PM_aistpcp90::DoDirichletInitializer(Dirichlet *dPtr)
{
  com();
...
void PM_aistpcp90::DoDirichletLoopStart(Dirichlet *dPtr)
{
  comment();
...
void PM_aistpcp90::DoDirichletCalcBoundaryValue(Dirichlet *dPtr)
{
  // used several times
...
void PM_aistpcp90::DoDirichletLoopEnd(Dirichlet *dPtr)
{
  com();
  writeSource("end do   !(boundary node loop)");
  com();

  writeSource("call PCP_MKBND0(NELT,neq,AMAT,VFG,ICOL0,ICPNT, &");
  writeSource("                nodes,NDF_PCP,INDB1,INDB2,BNDVAL)");
...
void PM_aistpcp90::doDirichletArgumentSequencePM(Dirichlet *dPtr)
{
...
void PM_aistpcp90::doIthDirichletCallInSolvePM(Solve *solvePtr,int i)
{
  comment();
  pushSource("! No.");
  pushSourceInt(i+1);
  ...
  pushSource("(NDF_PCP,no,firstNsetPtr,          &");
  flushSource();
...
void PM_aistpcp90::doIthDirichletCallInSolvePM2(Solve *solvePtr,int i)
{
  writeSource("end do");
  com();
}
