...
xplot::xplot(const char *displayName)
{
  xplotScreens++;
  xplotScreenNo = xplotScreens;


  std::cerr << "Open display "<<displayName << std::endl;
...
xplot::xplot(const char *displayName, int xSize, int ySize)
{
  xplotScreens++;
  xplotScreenNo = xplotScreens;
...
unsigned long xplot::xplotMycolor( const char *color_name)
{
  //    Colormap cmap;
  XColor c0,c1;
...
void xplot::xcolor( const char *color_name )
{
  //    Colormap cmap;
  XColor c0,c1;
  //    cmap = DefaultColormap(dpy,0);
