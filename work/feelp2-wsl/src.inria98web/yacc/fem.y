...
program            
  :       /* empty (initialize)?  */
  |   sections  { /* debug_start(); (テ擎ユ。ェfeel_exec_main、ホテ讀ヒーワ、キ、゛、キ、ソ。」)*/
                  feel_exec_main();
	          debug_terminate();
	          feel_terminate();  }
...
section
  : mesh_definition                 /* キチセチ */
  | element_definition              /* ヘラチヌトチ */
  | var_definition                  /* ハムソチ */
  | quadrature_definition           /* タムハャヒ。トチ */ 
  | scheme_definition               /* ・ケ・ュ。シ・狷チ */


/***********************************************/
/*  ヘラチヌトチ   elementハク 。ン。ン。ン。ン。ン。ン。ン。ン。ン。ン。ン*/
/***********************************************/
 /*  element ヘラチヌフセ [ゾキチヘラチヌ] {
 *        double  ハムソセ = ノセイチシー ;
 *        const   ハムソセ = ノセイチシー ;
 *        (コツノク,コツノク,...)     : エ豐愎;
...
cordinate
  : signed_num          { p_node_cordinate(1); } 
  | signed_num di num   { p_node_cordinate(2); }
...
point_definitions   /* point 文の本体 */
  : IDENTIFIER lb signed_num co signed_num rb { point_2d_st();}
  | point_definitions co IDENTIFIER lb signed_num co signed_num rb { point_2d_st();}
...
int_identifiers         /* 整数型変数の宣言 */
  ...
  | IDENTIFIER eq signed_num   { var_int_with_value_st(0); }
  | int_identifiers co IDENTIFIER { var_int_st(0);}
  | int_identifiers co IDENTIFIER eq signed_num { var_int_with_value_st(0); }

avsint_identifiers         /* 整数型変数の宣言(AVS) */
  : IDENTIFIER          { var_int_st(1); }
  | IDENTIFIER eq signed_num   { var_int_with_value_st(1); }
  | avsint_identifiers co IDENTIFIER { var_int_st(1);}
  | avsint_identifiers co IDENTIFIER eq signed_num { var_int_with_value_st(1); }

double_identifiers      /* 倍精度変数の宣言 */
  : IDENTIFIER          { var_double_st(0); }
  | IDENTIFIER eq signed_num   { var_double_with_value_st(0); }
  | double_identifiers co IDENTIFIER { var_double_st(0);}
  | double_identifiers co IDENTIFIER eq signed_num { var_double_with_value_st(0); }

avsdouble_identifiers      /* 倍精度変数の宣言(AVS) */
  : IDENTIFIER          { var_double_st(1); }
  | IDENTIFIER eq signed_num   { var_double_with_value_st(1); }
  | avsdouble_identifiers co IDENTIFIER { var_double_st(1);}
  | avsdouble_identifiers co IDENTIFIER eq signed_num { var_double_with_value_st(1); }
...
ewise_cordinate
  : signed_num di num  { push_ewise_cordinate_dat(2); }
  | signed_num         { push_ewise_cordinate_dat(1); }
  ...
  : signed_num                          
{
  ...
  | mi expression %prec UMINUS {exp_minus_push(); } /* 2026/4/11 */
  | expression mi expression   {exp_mi_push();    } /* 2026/4/11 */
  | IDENTIFIER                             /* シンボル */
{      expressions++;
...
/*  |  MESH       eq ON {push("mesh"); push("on"); settings++;} */
  ...
  |  setting_list co IDENTIFIER eq signed_num {settings++;}
...
num_parameter_list
  :  signed_num          { parameters++;}
  |  num_parameter_list co signed_num {parameters++;}
