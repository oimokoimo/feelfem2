 ...
 *            ~      ~       ~        ~
 *      File:   make_ecal_numerical.c
 *      Date:   1993/11/20
 *   
 *   Purpose:   数値積分法に対応するecal***ルーチンを作成する
 *   Functions: 
 *              
 *              
 */
     ...
     SolveElement *solve_element_ptr;
{
    ...
    FILE *fpw;     /* 2001/11/29 feelfem90vec */
    char  feelfem90vec[38];
    ...
    /* for feelfem90vec (2001/11/29)*/
    sprintf(feelfem90vec,"necal%d-%d-%d.dat",solve_no,elem_no,quad_no);
    fpw = fopen(feelfem90vec,"w");
    if(fpw==NULL) {
      fprintf(stderr,"Cannot open %s\n",feelfem90vec);
    }

    if(fpw!=NULL) fprintf(fpw,"%d\n",ndf);
