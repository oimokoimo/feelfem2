 ...
 *            ~      ~       ~        ~
 *      File:   elem_fort77_main_sk_nonlinear.c
 *      Date:   1993/07/12
 *   
 *   Purpose:   要素プログラムの作成(非線形問題、スカイライン行列対応版)
 *              
...
void elem_fort77_main_sk_nonlinear( no , solve_element_ptr )
     ...
     SolveElement *solve_element_ptr;   /* SolveElement 構造体  */
{
    ...
    switch(dimension) {
      case 1: 
	F77("(x,ielem,matno,nelem,np,VKGS,VKGD,VKGI,VFG,KLD,IPD");
	break;
      case 2:
	F77("(x,y,ielem,matno,nelem,np,VKGS,VKGD,VKGI,VFG,KLD,IPD");
	break;
      case 3:
	F77("(x,y,z,ielem,matno,nelem,np,VKGS,VKGD,VKGI,VFG,KLD,IPD");
	break;
    }
    ...
    F77("      dimension VKGS(*),VKGD(*),VKGI(*),VFG(*),KLD(*)\n");
    F77("      dimension IPD(*)\n");

    /* subdomain expansion */
    if(elem77_ptr -> subdomain_flag != NO) {
	F77("      dimension isubew(*)\n");
    }
...
/*
    ...
    */
    ...
    F77("          kp= ikp(k)\n");
    COM;
    F77("          if(kp .EQ. jp) goto 405\n");
    F77("          if(kp .GT. jp) goto 410\n");
    COM;
    F77("          lp = KLD(jp+1)-jp+kp\n");
    F77("          VKGI(lp) = VKGI(lp) + ea(j,k)\n");
    F77("          goto 400\n");
    COM;
    F77(" 405      VKGD(jp) = VKGD(jp) + ea(j,j)\n");
    F77("          VFG(jp)  = VFG(jp)  + eb(j)\n");
    F77("          goto 400\n");
    COM;
    F77(" 410      kp = ikp(k)\n");
    F77("          lp = KLD(kp+1)-kp+jp\n");
    F77("          VKGS(lp) = VKGS(lp) + ea(j,k)\n");
    COM;
    F77(" 400    continue\n");
    COM;
    F77(" 300  continue\n");
