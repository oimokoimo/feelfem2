 ...
 *            ~      ~       ~        ~
 *      File:   make_ncond_MPPSCG.c
 *      Date:   1996/04/22
 *   
 *   Purpose:   ノイマン条件ルーチン作成(並列解法版)
 *   Functions: 
 *              
...
void make_ncond_MPPSCG(solve_no,ncond_no,solve_ptr,n_ptr)
     ...
     NeumannData *n_ptr;
{
    ...
    /*********/
    /* gerogero */
    /* SystemWarning("make_ncond_MPPSCGはまだ製作途中です"); */
    /*********/
     ...
     *         
     */
    F77(",A,JA,B,ipdsub,npp_ibsubelem,matno,neukaz,");
    F77("neunum,nbedtno,feel,npmaxsub,nelemsub,neq_sub,");
    F77("MJ,JDP,neusub,NNC");
    ...
    F77("      dimension ipdsub(npmaxsub)\n");         
    F77("      dimension npp_ibsubelem(neusub),matno(nelemsub)\n");
    ...
    comment1(fp);
    PUT(fp,"      if(neukaz(%d)-1.gt.0) then\n",ncond_no);
    COM;
    PUT(fp,"         nnelemsub = neukaz(nbedtno(%d)-1)\n",ncond_no);
    PUT(fp,"         nenodesub = neunum(nbedtno(%d)-1)\n",ncond_no);
    COM;

    F77("         call ");
    PUT(fp,NCOND_CO_NAME,solve_no,ncond_no);
    F77("(x,y,A,JA,MJ,JDP,B,ipdsub,neq_sub,");
    PUT(fp,"feel(npp_ibsubelem(nbedtno(%d)-1)),",ncond_no);
    F77("matno,nnelemsub,nenodesub");
    ...
    sprintf(fname,NCOND_CO_FNAME,solve_no,ncond_no);
    StoreMakefile(fname,USER_LIBRARY);
    fp = OpenFileToWrite(fname);

        /*---
      subroutine文の作成(COルーチン部分)
      ---*/
    F77("      subroutine ");
    PUT(fp,NCOND_CO_NAME,solve_no,ncond_no);
     ...
     *         
     */
    F77(",A,JA,MJ,JDP,B,IPD,neq,ielem,matno,neelem,nenode");
...
/*
   ...
   */
	...
	COM;
	F77("         do 701 ii=1,JDP-1\n");
	F77("           if(kp.eq. JA(jp,ii)) THEN\n");
	F77("             A(jp,ii) = A(jp,ii) + ba(j,k)\n");
	F77("             goto 700\n");
	F77("           endif\n");
	F77(" 701     continue\n");
	F77("         stop 'NCOND ERROR'\n");
	COM;
	F77(" 705     A(jp,JDP) = A(jp,JDP) + ba(j,j)\n");
	F77("         B(jp)     = B(jp)     + bb(j)\n");
	F77("         goto 700\n");
	COM;
	F77(" 710     do 711 ii=JDP+1,MJ\n");
	F77("           if(kp.eq.JA(jp,ii)) THEN\n");
	F77("             A(jp,ii) = A(jp,ii) + ba(j,k)\n");
	F77("             goto 700\n");
	F77("           endif\n");
