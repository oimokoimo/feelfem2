 ...
 *            ~      ~       ~        ~
 *      File:   make_solve_ctrl77_sk_nonlinear.c
 *      Date:   1994/02/22
 *   
 *   Purpose:   solveブロックのコントロールfortranプログラムの生成(skyline版)
 *              非線形問題
 *   Functions: 
 *              
 ...
 */
	...
	PUT(fp,"(feel(np_VFG),feel(np_IPD),feel(np_ielem(%d)),n_edat(nedtno(%d),1),n_edat(nedtno(%d),2),feel(np_NON),npmax,DNORM",i+1,i+1,i+1);
	for(j=0;j<solve_element_ptr->unknowns;j++) {
	    F77(",");
	    PUT(fp,FEM_VAR_NAME77,*(solve_element_ptr->unknown_name + j));
	}
