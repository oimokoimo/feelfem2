...
void make_solve_ctrl77_CG( solve_ptr, solve77_ptr ,CG_type)
     Solve       *solve_ptr;
 ...
 */
    ...
    PUT(fp,"      call rep_CG(IPM(2),RPM(2))\n");
	...
	PUT(fp,"(feel(np_U),feel(np_IPD),feel(np_ielem(%d)),n_edat(nedtno(%d),1),n_edat(nedtno(%d),2)",i+1,i+1,i+1);
	for(j=0;j<solve_element_ptr->unknowns;j++) {
	    F77(",");
	    PUT(fp,FEM_VAR_NAME77,*(solve_element_ptr->unknown_name + j));
	}
