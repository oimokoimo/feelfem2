 ...
 *            ~      ~       ~        ~
 *      File:   make_dcond_CG_nonlinear.c
 *      Date:   1994/02/22
 *   
 *   Purpose:   非線形処理用ディリクレ条件ルーチン作成
 *   Functions: 
 *              
 *              この関数はmake_solve_fort77から呼ばれる
 *              
 *              非線形版は、１ ディリクレ条件をFEM変数に設定する
 *                          ２ ディリクレ節点に対応するΔｕを０にするよう
 *                             MATRIXを改造するに別れる
 *
 */
...
void make_dcond_CG_nonlinear(solve_no,dcond_no,d_ptr)
     ...
     DirichletData *d_ptr;
{
    ...
    F77("      dimension A(neq,MJ),JA(neq),B(neq)\n");
    F77("      dimension IPD(*)\n");         
    ...
    /* その２の処理の場合、行番号１０００に飛ぶ */
    F77("      IF(IDTYPE .EQ. 2) GOTO 1000\n");
    COM;
    /*====================*/
    /* 処理の始まり その１*/
    /*====================*/
