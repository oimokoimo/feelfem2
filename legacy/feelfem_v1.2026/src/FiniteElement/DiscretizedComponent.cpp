...
char *DiscretizedComponent::GetElementMatIJ(int n,int m)
{
  assert(0< n && n <= rows );
  assert(0< m && m <= columns );

  return (char *)(elementStiffMatPtr->val(n-1,m-1));
}

char *DiscretizedComponent::GetElementLoadI(int n)
{
  assert(0< n && n <= rows );
  return (char *)((*rhsVecPtr)[n-1]);
}
...
void DiscretizedComponent::print(ostream &ost)
{
  ost << "DiscretizedComponent  ("<<rows <<"," << columns<<")" << endl;
  ...
  for(int i=0;i<rows;i++) {
	...
	ost << "(" << i+1 << "," << j+1 << ") ";
      ...
      ost << elementStiffMatPtr->val(i,j) << endl;
    }
    ost << "----------------------------------------------------" << endl;  
    ost << "RHS[" << i+1 << "] " << (*rhsVecPtr)[i] << endl;
    ost << "----------------------------------------------------" << endl;  
    ost << "----------------------------------------------------" << endl;  
  }
