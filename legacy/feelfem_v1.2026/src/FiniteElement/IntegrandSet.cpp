...
{
  if(quadPtr == 0) {
    cerr << "IntegrandSet::quadPtr == 0" << endl;
    assert(1==0);
  }
  return(quadPtr);
}  

void IntegrandSet::print(ostream &ost)
{
  ost << "--------------------------------" << endl;
  ost << "IntegrandSet :: equations = " << equations << endl;
  ost << "Quadrature method           " << quadPtr->GetName() << endl;
