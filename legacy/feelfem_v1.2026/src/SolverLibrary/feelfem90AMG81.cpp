...
#include "LIB_feelfem90AMG81.hpp"
...
void LIB_feelfem90AMG81 <MT_ff90AMGCRS>::SolveCallSolverRoutine(Solve *solvePtr)
{
  ...
  pushSource(" call preportmodule('AMG solve No.");
...
GenerateCoSolveSolverParamRoutine(Solve *solvePtr)
{
  ...
  pushSource("module mod_amg81param");
  pushSourceInt( solvePtr->GetSolveNo());
  pushSource("_");
  ...
  pushSource("subroutine amg81param");
  pushSourceInt( solvePtr->GetSolveNo());
  pushSource("_");
  pushSourceInt(solveElementNo);
  ...
  if(ivars < 2) {
    ...
    flushSource();
    writeSource("npts = 0            ! ipamg usage indicator"); 
