...
int meshEvalGinac::meshAssignment(SNmeshAssignment *dPtr)
{
  int errFlag;
  errFlag = 0;
...
int meshEvalGinac::meshPoint(SNpoint *dPtr)
{
  int errFlag;
  errFlag = 0;
  ...
  if(errFlag == 0) {
    errFlag = dPtr->evalCoordinate(&evalList);
...
int meshEvalGinac::isAllAssingedEvalPairList(list <string> &nonEvaledLst)
{
  int errFlag;
  errFlag = 0;
...
evalPair *meshEvalGinac::getEvalPairByString(string &nm)
{
  listIterator <evalPair *>itr(evalList);
