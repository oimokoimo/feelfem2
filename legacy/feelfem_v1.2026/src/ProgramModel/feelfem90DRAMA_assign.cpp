 ...
 *
 *  Filename : feelfem90DRAMA_assign.cpp
 *  Date     : 2003/02/07 (copied from feelfem90smpi)
 *  Modified : 
 *  
...
char *PM_feelfem90DRAMA::GetAssignRoutineName(int n)
{
  assert(n>0);
...
void PM_feelfem90DRAMA::pushAssignRoutineName(Assign *asPtr)
{
  pushSource("let");
...
// CO-ROUTINE
char *PM_feelfem90DRAMA::GetAssignCoRoutineName(int n)
{
  assert(n>0);
...
// CO-ROUTINE
void PM_feelfem90DRAMA::pushAssignCoRoutineName(Assign *asPtr)
{
  pushSource("colet");
  pushSourceInt(asPtr->GetAssignNo());
  return;
...
void PM_feelfem90DRAMA::DoAssignCoRoutineReturnSequence(Assign *asPtr)
{
  writeSource("return");
  com();
...
// main routine
void PM_feelfem90DRAMA::DoMainScheme_assign(Main *mPtr, Assign *asPtr)
{
  ...
  writeSource("!--------------------------------------------------");
  writeSource("! feelfem90DRAMA, all processors have to calculate ");

  // In feelfem90DRAMA model, all processor calculate its integral 
  // contribution
  if( asPtr->GetLeftVarAssignType() == AS_VAR_TYPE_SCALAR ||
      asPtr->GetGeometryType()      == ASSIGN_TYPE_INTEGRAL     ) {
    writeSource("!");
    writeSource("!  integral operator needs special care for DRAMA");
  }
...
// assign subroutine
void PM_feelfem90DRAMA::DoAssignRoutineHeader (Assign *asPtr)
{
  // module sentence
...
void PM_feelfem90DRAMA::DoAssignParameters( Assign *asPtr)
{
...
void PM_feelfem90DRAMA::DoAssignVariableDefinitions(Assign *asPtr)
{
...
void PM_feelfem90DRAMA::DoAssignGetGeometryData(Assign *asPtr)
{

  writeSource("not yet doassigngetgeometrydata");
}

void PM_feelfem90DRAMA::DoAssignStatement(Assign *asPtr)
{
...
void PM_feelfem90DRAMA::DoAssignReturnSequence(Assign *asPtr)
{
  writeSource("return");
  com();
...
void PM_feelfem90DRAMA::doAssignScalar(Assign *asPtr)
{
  ...
  for(itrV.init(); !itrV ; ++itrV) {
    ...
    case VAR_MATERIAL:
      cerr << "material variable must not be appeared here.\n";
      assert(1==0);  
...
void PM_feelfem90DRAMA::doAssignFEMnode(Assign *asPtr)
{
  // do loop
...
void PM_feelfem90DRAMA::doAssignEWISEmaterial(Assign *asPtr)
{
  // do loop
