...
{
  cout << "Compareing SolveElements, its may be wrong!\n";
  exit(1);
  return(0);
}
...
Quadrature *SolveElement::GetIthQuadraturePtr(int i)
{
  if(i < 0 || i >= quadratures) {
    cerr << "Illegal no indicated in SolveElement::GetIthQuadraturePtr\n";
    exit(1);
  }
  ...
  cerr << "Illegal in SolveElement::GetIthQuadraturePtr\n";
  exit(1);
}
...
{
  if(i < 0 || i >= ndf) {
    cerr << "Illegal i in SolveElement::GetIthIEDP\n";
    exit(1);
  }
  return(*(iedp+i));
}
...
{
  if(i < 0 || i >= ndf) {
    cerr << "Illegal i in SolveElement::GetIthIENP\n";
    exit(1);
  }
  return(*(ienp+i));
}
