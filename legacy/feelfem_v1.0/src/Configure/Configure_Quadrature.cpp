...
Quadrature *Configure::GetDefaultQuadraturePtrByEtype( int etype )
{
  ...
  case ELEMENT_TYPE_POINT:
    cerr << "Configure::GetDefaultQuadraturePtrByEtype for POINT is called.\n";
    assert(1==0);
  ...
  default:
    cerr << "Illegal etype code at GetDefaultQuadraturePtrByEtype." << endl;
    assert(1==0);
  }

  if(feelfemobj.IsDefinedQuadratureName( name ) == NO) {
    cerr << "Default quadrature for etype " << etype <<" is "<<
      name << ", but not defined." << endl;
...
Quadrature *Configure::GetDefaultBoundaryQuadFromRegionEtype(int etype)
{
  ...
  if(feelfemobj.IsDefinedQuadratureName( name ) == NO) {
    cerr << "Default boundary quadrature for etype " << etype <<" is "<<
      name << ", but not defined." << endl;
