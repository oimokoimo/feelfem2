...
Source::Source( char *name)
{
  ...
  //  if(fpSource == NULL) {
  //    cerr << "Cannot open " << FEELFEM_SRC_FNAME << " to create.\n";
  //    abortExit(1);
  //  }
   //  cerr <"DEBUG IN SOURCE(CHAR *)\n";  for debug
...
{
  if(fpSource == NULL) {
    cerr << "File pointer is NULL in Source::GetSourceFP() \n";
    abortExit(1);
  }
  return(fpSource);
}
...
void Source::startSource(char *str)       // this is called OpenSource 
{
  if(divideFlag == 1 && fpSource != NULL) {
    cerr << "fpSource is not NULL while calling startSource()\n";
    abortExit(1);
  }

  fpSource = fileOpenToWrite(str);
  if(fpSource == NULL) {
    cerr << "Cannot open " <<str << " to write.\n";
    abortExit(1);
  }
...
void Source::startSource(char *str , int makeFlag)
{
  if(divideFlag == 1 && fpSource != NULL) {
    cerr << "fpSource is not NULL while calling startSource()\n";
    abortExit(1);
...
{
  if(fpSource == NULL) {
    cerr <<"Try to close NULL pointer in endSource()\n";
    abortExit(1);
  }
  fclose(fpSource);
  fpSource = NULL;
