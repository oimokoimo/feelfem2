...
double evalNodeTri(double xi,double eta)
{
  ...
  if(abs((xi + eta-1.0))<EVALNODE_EPS) {
    return( 5.0 -  xi);
  }
...
{
  // mid point
  if(xi == 0.25 && eta == 0.25 && zeta == 0.25) return(1.0);
  ...
  if(abs((xi + eta-1.0))<EVALNODE_EPS && zeta == 0.0) {
    return(12.0+ (1.0  -  xi) );
  }
  ...
  if(abs((xi+zeta-1.0))<EVALNODE_EPS && eta == 0.0) {
    return(18.0 + zeta);
  }

  // Edge No.3 to No.4 (xi = 0,etai+zeta = 1.0  20.0 < val < 21.0
  if(abs((eta+zeta-1.0))<EVALNODE_EPS && xi == 0.0) {
    return(20.0 + zeta);
  }
  ...
  cerr << "Illegal node position in Tetrahedra\n";
  cerr << "xi   = " << xi   <<endl;
  cerr << "eta  = " << eta  <<endl;
  cerr << "zeta = " << zeta <<endl;
...
{
  cerr<< "FiniteElement/evalNodeOrder.cpp evelNodeCube() is underconstruct\n";
  exit(1);
}
