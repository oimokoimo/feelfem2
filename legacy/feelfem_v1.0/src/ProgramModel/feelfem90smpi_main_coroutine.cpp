 ...
 *
 *  Filename : feelfem90smpi_main_coroutine.cpp
...
void PM_feelfem90smpi::GenerateMainCoRoutine(Main *mainPtr)
{
...
void PM_feelfem90smpi::GenerateMakefileForUserFunction()
{
  listIterator <Function *>itr(feelfemobj.GetUserFunctionList());
  for(itr.init();!itr ;++itr) {
    char *sourceName = GetSourceName( itr()->GetName());
    StoreSourceUniq(sourceName, LINKLEVEL_USERFUNC);
  }
  return;
}

void PM_feelfem90smpi::GenerateMaterialVarInitializeRoutine(void)
{
