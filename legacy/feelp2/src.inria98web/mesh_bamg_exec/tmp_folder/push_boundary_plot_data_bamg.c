     ...
     int   numdatno;
{
	...
	bd_ptr = get_b2d_ptr_by_both_ends(from,to);
	
	total_points = total_points + bd_ptr ->points;
    }
	...
	if(streq(bd_ptr->from_name,from)) {
	    /* 正の向き */
	    if(i == 0) {
		node_ptr = bd_ptr -> node_no;
		points   = bd_ptr -> points;
	    }
	    else {
		node_ptr = bd_ptr -> node_no;
		node_ptr++;                     /* 最初はスキップ */
		points   = bd_ptr -> points -1; /* 最初はスキップ */
	    }
