 ...
 *            ~      ~       ~        ~
 *      File:   make_MESH_REF.c
 *      Date:   1993/10/14
 *   
 *   Purpose:   MESH.REFファイルの作成
 *              
 *              唯一のdomainに対してのメッシュ生成
 *              領域に関するデータは、../solve_comp/edat_make.c内の情報を
 *              用いる
 */
...
make_MESH_REF( mesh_edat_ptr , total_ptr )
     ElemDat *mesh_edat_ptr;
     MidPoint2D *total_ptr;
{
    int i,j,k;

    FILE        *fp;             /* MESH.REF作成用のファイルポインタ */
    ...
    fp = OpenFileToWrite("MESH.REF");
    StoreDeletefile( "MESH.REF" );
