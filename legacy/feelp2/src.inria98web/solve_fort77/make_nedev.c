     ...
     SolveElement *solve_element_ptr;
{
	...
	COM;
	PUT(fp,"      do %d j=1,%d\n",110+i,freedom);
	PUT(fp,"       zansa = VFG(IPD(ielem(ienp_%s(j),i))+iedp_%s(j))\n",
	    var_name,var_name);
	PUT(fp,"       dat   = fem_%s(ielem(ienp_%s(j),i))\n",
	    var_name,var_name);
	PUT(fp,"       work(ielem(ienp_%s(j),i),%d) = zansa\n",var_name,i+1);
	PUT(fp,"       DNORM  = DNORM  + zansa*zansa\n");
	PUT(fp,"       ZNORM  = ZNORM  + dat  *dat  \n");
