...
void Showvec_file_st() 
{
    ...
    showvec_struct -> filename  = PopString();  /* ファイル名の取得 */
    /* DEFAULT値の設定 */
    showvec_struct -> winsiz = DEFAULT_WINDOW_SIZE;  /* window size  */
    showvec_struct -> USE_EDATRANGE_flag = YES;
    showvec_struct -> IFLG   =   2;  /* VMAXのみ自動設定 */
    showvec_struct -> xmin   = 0.0;  /* 自動設定する */
    ...
    store_scheme( SHOWVEC_FILE , showvec_struct);
...
void Showvec_file_st_with_setting(settings) 
     int settings;
{
    int i;
    char    *var;
