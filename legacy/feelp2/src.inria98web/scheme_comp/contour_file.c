...
void Contour_file_st() 
{
    ...
    var = PopString();
    contour_struct -> filename = PopString();  /* ファイル名 */
    contour_struct -> file_io_no = get_data_file_number();


    contour_struct -> var  = var;

    contour_struct -> yylineno  = yylineno_info();
    contour_struct -> window_id = get_new_file_window_id();
    ...
    contour_struct -> gray_flag  = 0;
    contour_struct -> mesg = NULL;
    contour_struct -> subdomain = NULL;

    store_scheme( CONTOUR_FILE , contour_struct);
...
void Contour_file_st_with_setting(settings) 
     int settings;
{
    int i;
    char    *var;
	...
	if(streq(key,"mesh")) {
	    ...
	    if(streq(num,"off")) {
		contour_struct->mesh_flag = 1; /* メッシュを描かない */
		continue;
	    }
	    SystemError_yy("contour文キーワードmeshにはon/offを指定",
			 yylineno_info());
	    continue;
	}
	...
	if(streq(key,"mono")) {
	    ...
	    SystemError_yy("contour文キーワードmonoにはon/offを指定",
			 yylineno_info());
	    continue;
	}

	SystemError_yy_s("contour文詳細指定キーワード%sはだめ",key,
			 yylineno_info());

    }
    var = PopString();
    contour_struct -> filename = PopString();  /* ファイル名 */
    contour_struct -> file_io_no = get_data_file_number();  /* ファイルIO番号 */
	...
	if( ymax <= ymin || xmax <= xmin) {
	    SystemError_yy("contour表示領域指定に誤りがある",yylineno_info());
	}
    }
    if(urange_flag != NO) {
	if( umax <= umin ) {
	    SystemError_yy("contour値範囲指定に誤りがある",yylineno_info());
	}
    }
