 ...
 */
    F77(   "      stop 'まだ作っていません(make_read_solve_data77)'\n");
    ...
    F77(   "* ------------------\n");
    PUT(fp,"      call f_alloc(np_IPD,'solve_%d(IPD)',npmax,0)\n",solve_no);

    /* 節点自由度配列IPF */
    F77(   "* ---\n");
    F77(   "* IPF (node-wise freedom)\n");
    F77(   "* ------------------\n");
    PUT(fp,"      call f_alloc(np_IPF,'solve_%d(IPF)',npmax,0)\n",solve_no);
    ...
    PUT(fp,"        call f_alloc(np_ielem(i),'solve_%d(ielem:1)',neelem * nenode,0)\n",solve_no);
    PUT(fp,"        call f_alloc(np_matno(i),'solve_%d(matno:1)',neelem ,0)\n",solve_no);
    /* ielemの取得 */
	...
	PUT(fp,"        call f_alloc(np_bielem(i),'solve_%d(bielem)',neelem * nenode,0)\n",solve_no);
	PUT(fp,"        call f_alloc(np_bmatno(i),'solve_%d(bmatno)',neelem ,0)\n",solve_no);
	/* ielemの取得 */
    ...
    PUT(fp,"      call f_alloc(np_KLD,'solve_%d(KLD)',neq+1,0)\n",solve_no);

    /* スカイライン列の高さ用配列、補助配列のメモリ取得 */
    PUT(fp,"      call f_alloc(np_KH, 'solve_%d(KH)', neq,  0)\n",solve_no);
    PUT(fp,"      call f_alloc(np_KHa,'solve_%d(KHa)',npmax,0)\n",solve_no);
 ...
 *    COM; */
    ...
    F77(   "* VFG(荷重ベクトル)\n");
    PUT(fp,"      call f_alloc(np_VFG,'solve_%d(VFG)',neq,1)\n",solve_no);
    COM;

    /* 配列VKGS(上三角) VKGI(下三角) VKGD(対角項) の領域取得 */
    F77(   "* VKGS(上三角) VKGI(下三角) VKGD(対角項)\n");
    PUT(fp,"      call f_alloc(np_VKGS,'solve_%d(VKGS)',ncof,1)\n",solve_no);
    PUT(fp,"      call f_alloc(np_VKGI,'solve_%d(VKGI)',ncof,1)\n",solve_no);
    PUT(fp,"      call f_alloc(np_VKGD,'solve_%d(VKGD)',neq, 1)\n",solve_no);
    COM;
	...
	PUT(fp,"       call f_alloc(np_inset(i),'solve_%d(INSET)',nodes*np,0)\n",solve_no);
